  ...
  return (
    <>
      ...
      <ModalShell
        ...
        description="Create the account here, assign the cohort, then copy the login details once for the manager."
      >
        <form action={action} className="grid gap-3 md:grid-cols-2">
          ...
          {state.message ? (
            <p className={`text-sm ${state.ok ? "text-emerald-700" : "text-rose-700"} md:col-span-2`}>{state.message}</p>
          ) : null}

          {state.credentials ? (
            <div className="space-y-3 rounded-2xl border border-slate-200 bg-white p-4 md:col-span-2">
              <p className="text-sm font-medium text-slate-950">Copy these login details now</p>
              <CopyRow label="Email" value={state.credentials.email} />
              <CopyRow label="Temporary password" value={state.credentials.password} />
              <CopyRow label="Login URL" value={state.credentials.loginUrl} />
            </div>
          ) : null}
